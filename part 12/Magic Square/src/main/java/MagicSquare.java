...
public class MagicSquare {
    ...
    public ArrayList<Integer> sumsOfRows() {
        ...
        for (int[] square1 : this.square) {
            int sum = 0;
            for (int column = 0; column < square1.length; column++) {
                sum = sum + square1[column];
            }
            sums.add(sum);
        }
        return sums;
    ...
    public ArrayList<Integer> sumsOfDiagonals() {
        ...
        for (int row = 0; row < this.square.length; row++) {
            for (int column = 0; column < this.square[row].length; column++) {
                if (row == column) {
                    left = left + this.square[row][column];
                }
                if (row + column == this.square.length - 1) {
                    right = right + this.square[row][column];
                }
            }
        }
        sums.add(left);
